package realmproxy.net;

import java.net.InetSocketAddress;
import java.net.ServerSocket;
import java.net.Socket;
import java.net.SocketAddress;

public abstract class ListenSocket implements Runnable {

	private final SocketAddress destination;
	private ServerSocket serversocket;
	private Thread thread;
	
	public ListenSocket(InetSocketAddress destination){
		this.destination=destination;
	}
	
	public ListenSocket(int port){
		this(new InetSocketAddress(port));
	}
	
	public ListenSocket(String hostname, int port){
		this(new InetSocketAddress(hostname, port));
	}
	
	public boolean isClosed() {
		return this.serversocket == null || this.serversocket.isClosed();
	}
	
	public boolean start() {
		stop();
		try {
			this.serversocket = new ServerSocket();
			this.serversocket.bind(this.destination);
		} catch (Exception e) {
			e.printStackTrace();
			stop();
			return false;
		}
		this.thread = new Thread(this);
		this.thread.start();
		return true;
	}

	public void stop() {
		this.thread = null;
		if (this.serversocket != null) {
			try {
				this.serversocket.close();
			} catch (Exception e) {
				e.printStackTrace();
			}
			this.serversocket = null;
		}
	}
	
	@Override
	public void run() {
		while (this.serversocket != null && !this.serversocket.isClosed()) {
			try {
				final Socket socket = this.serversocket.accept();
				new Thread(new Runnable() {

					@Override
					public void run() {
						socketAccepted(socket);
					}
					
				}).start();
				
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	}

	public abstract void socketAccepted(Socket socket);
	
	@Override
	public String toString() {
		return "ListenSocket[destination=" + this.destination + "]";
	}
}
